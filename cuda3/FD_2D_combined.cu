...
int main(int argc, char* argv[])
{
	...
	if((strcmp(argv[2], "global") == 0)) {
	        ...
	        std::cout<<"second argument should be global|shared|texture"<<std::endl;
	        exit(-1);
	}
	...
	cudaMemcpy(u, u_d, N*N*sizeof(float), cudaMemcpyDeviceToHost);
        ...
        {	for (int i=0; i<N; i++)
	        {	I = N*j + i;
	                if(fabs(u_cpu[I] - u[I]) > 0.3f) {
	                        std::cout<<"not equal"<<std::endl;
	                        std::cout<<u_cpu[I]<<" "<<u[I]<<" "<<fabs(u_cpu[I] - u[I])<<std::endl;
	                        exit(-1);
	                }
	        }
	}
	std::ofstream temperature2("temperature_cpu");
	for (int j=0; j<N; j++)
	{	for (int i=0; i<N; i++)
		{	I = N*j + i;
		        temperature2<<x[I]<<"\t"<<y[I]<<"\t"<<u_cpu[I]<<std::endl;
		}
		temperature2<<"\n";
	}
	

        char* prefix = "temperature_";
        char* suffix = ".txt";
        char all[40];
