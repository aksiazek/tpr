...
def parse_input():
	if len(sys.argv) < 2:
		print 'Usage ./capacity <std|sync|buff> [number_of_messages]'
		sys.exit(1)
	...
	type = sys.argv[1]
	return (number_of_messages, type)

BITS_IN_BYTE = 8
NUMBER_OF_BITS_IN_MBITE = 10**6

number_of_messages, type = parse_input()
...
for message_size in [2 ** x for x in range(3, 21)]:
	...
	if rank == 0:
		...
		results.append((transfered_data / result_time) / NUMBER_OF_BITS_IN_MBITE)
	else:
